...
function createDeepProxy(target: any, path: string[] = []) {
  return new Proxy(target, {
    get(obj, prop, receiver) {
      console.log(
        `[GET] Accessed property: '${String(prop)}' (Path: ${[...path, String(prop)].join(".")})`,
      );
    ...
    deleteProperty(obj, prop) {
      console.log(
        `[DELETE] Deleted property: '${String(prop)}' (Path: ${[...path, String(prop)].join(".")})`,
      );
    ...
    set(obj, prop, newValue, receiver) {
      console.log(
        `[SET] Changed property: '${String(prop)}' to '${newValue}' (Path: ${[...path, String(prop)].join(".")})`,
      );
